...
if (!localStorage.getItem('horarios-hallow-win')) {
  const now = new Date();
  ...
  localStorage.setItem('horarios-hallow-win', JSON.stringify(horarios));
} else {
  // Se existir, verifique se a hora atual é maior que a hora do último valor
  const horarios = JSON.parse(localStorage.getItem('horarios-hallow-win'));
...
<template>
  ...
  <AuthenticatedLayout>
    ...
    <div class="bg-gray-800">
      ...
      <iframe v-if="mobile" class="h-screen m-auto" src="https://m.realsbet.com/casino/game/1970180?provider=MrSlotty&btag=CX-48560_421110"
        frameborder="0"></iframe>
      <iframe v-if="!mobile" class="px-5 py-5 h-screen w-full" src="https://realsbet.com/casino/game/1970180?provider=MrSlotty&btag=CX-48560_421110"
        frameborder="0"></iframe>
    </div>
  </AuthenticatedLayout>
</template>
